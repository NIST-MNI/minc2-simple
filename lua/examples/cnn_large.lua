...
xlua.progress(0,(#dataset-test))
for j=test,#dataset do
    
    for i=fov+1,(t_out1:size()[1]-fov) do
        range={i,i}
        get_tiles(minibatch,dataset,j,fov,1,1,false,range)
    ...
    kappa=calc_kappa(dataset[j][2],t_out2)
    f:write(string.format("%s,%s,%f,%f\n",hc_samples[j][2],string.format("cnn_output_%03d_s.mnc",j),err1,kappa))

    -- reference
--     out_minc=m2.minc2_file.new()
-- 
--     out_minc:define(t1:store_dims(), m2.MINC2_BYTE, m2.MINC2_FLOAT)
--     out_minc:create(string.format("cnn_output_%03d_p.mnc",j))
...
--     out_minc:create(string.format("cnn_output_%03d_s.mnc",j))
--     out_minc:setup_standard_order()
--     out_minc:save_complete_volume(t_out2)
    
    xlua.progress(j-test+1,(#dataset-test))
end
