...
train=120    -- use first N subjects for training 
...
local function put_tiles_max(ds, out, fov, stride,mult)
    ...
    local out_t=torch.ByteTensor(volume_sz):fill(1)
...
for i=test+2,#dataset do
    
    get_tiles(minibatch,dataset,i,fov,stride,1,false)
    
    out1=mlp:forward(minibatch[1])
    err1=criterion:forward(out1, minibatch[2])
    print(string.format("Error on test dataset: %d %e",i,err1))
    print(out1:size())
    
    t_out1=put_tiles(dataset[1],out1,fov,stride,1,2)
    ...
    out_minc:create(string.format("mlp_output_%03d_p.mnc",i))
    ...
    out_minc:create(string.format("mlp_output_%03d_s.mnc",i))
    out_minc:setup_standard_order()
